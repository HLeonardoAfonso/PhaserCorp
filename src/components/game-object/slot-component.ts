...
export class Slot {
  ...
  /** Add 1 item. Stacks if same item key, refuses if occupied by different item. */
  addOne(itemKey: string): boolean {
    // Stack onto existing matching item
    if (this.#itemKey === itemKey && this.#amount < 64) {
      this.#amount++;
      this.#amountText?.setText(`${this.#amount}`);
      return true;
    }

    // Refuse if occupied by a different item
